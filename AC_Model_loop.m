...
%         'with Vector Strength = ' num2str(vs_EE4)]);
    ...
    % correlation between A1 and R (temporal / spectral)            (57/58)
    % correlation between Slow and Fast (temporal / spectral)       (59/60) 
    % correlation between A1 and Slow (temporal / spectral)         (61/62)
    % correlation between R and Fast (temporal / spectral)          (63/64)
    % correlation between temporal and spectral (A1,R,Slow,Fast)    (65/66/67/68)
    ...
    corr = corrcoef(mean(EE1,2),mean(EE2,2));  sound_data(i,58) = corr(2);
    corr = corrcoef(mean(EE3,1),mean(EE4,1));  sound_data(i,59) = corr(2);
    corr = corrcoef(mean(EE3,2),mean(EE4,2));  sound_data(i,60) = corr(2);
    corr = corrcoef(mean(EE1,1),mean(EE3,1));  sound_data(i,61) = corr(2);
    corr = corrcoef(mean(EE1,2),mean(EE3,2));  sound_data(i,62) = corr(2);
    corr = corrcoef(mean(EE2,1),mean(EE4,1));  sound_data(i,63) = corr(2);
    corr = corrcoef(mean(EE2,2),mean(EE4,2));  sound_data(i,64) = corr(2);
    corr = corrcoef(downsample(mean(EE1,1),164),mean(EE1,2));  sound_data(i,65) = corr(2);
    corr = corrcoef(downsample(mean(EE2,1),164),mean(EE2,2));  sound_data(i,66) = corr(2);
    corr = corrcoef(downsample(mean(EE3,1),164),mean(EE3,2));  sound_data(i,67) = corr(2);
    corr = corrcoef(downsample(mean(EE4,1),164),mean(EE4,2));  sound_data(i,68) = corr(2);
