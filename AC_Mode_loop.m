...
for mode=modes
    ...
    for i=1:numSounds
        ...
        if hearingLossMode
            ...
            aud_freq = [250, 500, 1000, 2000, 3000, 4000, 6000, 8000]; %HZ
            aud_hl = [90, 95, 95, 97.5, 97.5, 107.5, 112.5, 110]; %dB HL (patient-specific)
            if  afterCI_mode
                aud_hl = [55, 50, 30, 15, 20, 10, 32.5, 25]; %dB HL afterCI (patient-specific)
            end
        ...
        %temporal
        sound_data(i,15) = std(mean(EE4,1));
        sound_data(i,16) = max(mean(EE4,1));
        ...
        sound_data_small = sound_data(:,[1, 8:13, 14:20, 27:124]);
    ...
    if hearingLossMode
        if afterCI_mode
            save('soundData_afterCI.mat', "sound_data")
            save('soundData_afterCI_small.mat', "sound_data_small")
        else
            save('soundData_HL.mat', "sound_data")
            save('soundData_HL_small.mat', "sound_data_small")
        end
    elseif CI_mode
        save('soundData_CI.mat', "sound_data")
        save('soundData_CI_small.mat', "sound_data_small")
    else
        save('soundData.mat', "sound_data")
        save('soundData_small.mat', "sound_data_small")
    end
end
